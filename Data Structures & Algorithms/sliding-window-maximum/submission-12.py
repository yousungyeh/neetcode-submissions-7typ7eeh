class Solution:
    def maxSlidingWindow(self, nums: List[int], k: int) -> List[int]:
        """
        Optimal Solution: deque to save the index for sliding window
        Core Concept: deque need to maintain descending order, 
        that is, dq[0] is maximum index in current window
        """
        dq = deque()  # 存 index
        ans = []
